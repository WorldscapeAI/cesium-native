...
macro(EZVCPKG_CHECK_RESULTS)
    if (NOT (EZVCPKG_RESULT EQUAL 0))
        if (EZVCPKG_IGNORE_ERRORS)
            ...
            message(SEND_ERROR "EZVCPKG failed with error ${EZVCPKG_RESULT}")

            message("*** The output from the command was:")
            message("${EZVCPKG_OUTPUT}")

            if (EZVCPKG_OUTPUT MATCHES "See logs for more information:\r?\n([^\r\n]+\r?\n)*")
                string(REGEX MATCHALL "\r?\n([^\r\n]+)" LOGS "${CMAKE_MATCH_0}")
                foreach(LOG IN LISTS LOGS)
                  string(STRIP "${LOG}" LOG_FILENAME)
                  if(EXISTS "${LOG_FILENAME}")
                      message("*** Content of log file: ${LOG_FILENAME}:")
                      file(READ "${LOG_FILENAME}" LOG_CONTENT)
                      message("${LOG_CONTENT}")
                  else()
                    message("*** Could not find log file: ${LOG_FILENAME}")
                  endif()
                endforeach()
            endif()
...
macro(EZVCPKG_BUILD)
    ...
    if (DEFINED VCPKG_OVERLAY_PORTS)
        if (CMAKE_HOST_WIN32)
            ...
            string(REPLACE ";" ":" VCPKG_OVERLAY_PORTS_POSIX "${VCPKG_OVERLAY_PORTS}")
            set(ENV{VCPKG_OVERLAY_PORTS} "${VCPKG_OVERLAY_PORTS_POSIX}")
        endif()
    endif()
    ...
    if (DEFINED VCPKG_OVERLAY_TRIPLETS)
        if (CMAKE_HOST_WIN32)
            ...
            string(REPLACE ";" ":" VCPKG_OVERLAY_TRIPLETS_POSIX "${VCPKG_OVERLAY_TRIPLETS}")
            set(ENV{VCPKG_OVERLAY_TRIPLETS} "${VCPKG_OVERLAY_TRIPLETS_POSIX}")
        endif()
    endif()
...
function(EZVCPKG_FETCH_IMPL)
    ...
    set(EZVCPKG_PACKAGES_DIR "${EZVCPKG_DIR}/packages" PARENT_SCOPE)
